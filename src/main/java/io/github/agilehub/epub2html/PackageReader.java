...
import static io.github.agilehub.epub2html.EpubPaths.normalize;
 ...
 *
 * <p>由流水线先调用 packagePath 定位 OPF，再调用 readPackage 读取它。
 * manifest 类似文件索引表，spine 类似阅读顺序表。本类不解析正文，也不处理媒体字节。
 */
final class PackageReader {
   ...
   * @return 第一个 rootfile 的 full-path 经整理后的路径
   ...
   *
   * <p>manifest 把资源 ID 映射为路径；spine 使用 ID 指定正文顺序。本实现跳过无法映射的 itemref，不过滤 linear 属性；多个 NCX/nav 声明时保留最后一次匹配。
   *
  ...
  static PackageData readPackage(ZipFile zip, String opfPath) throws IOException {
    ...
    // manifest 的 id 是 spine 引用的“键”，值是相对于 OPF 解析后的文件路径。
    ...
    // 第二轮按 spine 的 itemref 顺序取路径，清单中的排列顺序不决定正文顺序。
